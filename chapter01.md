...
macOS 具有非常友好的视窗操作界面，并且与硬件配合堪称天衣无缝（毕竟是同一家生产的），这赢得了大众的欢迎（只是价格不怎么亲民）。对于开发者而言，图形化操作当然是必须的。除此之外，前述的“手指不离开键盘”完成各种操作，在 macOS 中也能轻松实现，因为 macOS 是基于 Unix 的，它与 Linux 同宗同源，所以常用的 Linux 命令可以用在 macOS 上。

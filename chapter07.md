...
不论 Python 的内置函数还是模块中提供的函数（比如 `math.pow()` ），都是“别人”创造的，我们施行“拿来主义”，只是应用。但是，当“别人”定义的函数不能满足自己的需要时，就必须要“自力更生”，自己定义函数了。
...
对照图7-1-1所示的函数定义形式，这里定义的函数 `add()` 符合该形式要求，若与注释（1）所定义的函数相比，不仅语句块中的逻辑行数量多了，还增加了“函数文档”

- 在函数里面，用三个引号包裹的多行注释，称之为“函数的文档”。通常函数文档中编写有关函数的说明，如函数的作用，调用方法及返回值等——类似于 `.py` 程序文件开头的文档。
- 函数 `add()` 的语句块中有两个语句，其中 `return r` 称为 return 语句。`return` 是 Python 的关键词，它的作用是将变量 `r` 引用的对象返回到函数调用的位置。有的函数没有显示地写出 renturn 语句，Python 解释器会默认 `return None` 。例如前面编写的函数 `my_func()` ：
...
函数 `laoqi()` 的 `return` 后面是一个字典对象，注释（6）中 `laoqi()` 即调用此函数，并将字典对象返回到此位置，即得到了 `{'work': 'coder', 'age': 30, 'gender': 'male'}['work']` ，从而读取到键 `'work'` 的值 `'coder'` 。
...
根据此数学式得到的数列，如：$0,1,1,2,3,5,8,13,21,\cdots$​​ ，就是斐波那契数列。接下来的任务就是研究用 Python 编写计算斐波那契数列的函数。
...
函数 `fibo_loop(0` 中用循环语句实现了斐波那契数列（读者也可以将其中的 for 循环改为 while 循环）。这种方法简单，而且直接与数学表达式对应，容易理解。除了此方法之外，更常见的或许是用“递归”的方法来实现。
...
- 将 `lst` 传给函数 `bar(lst)` 的本质就是参数 `a` 与变量 `lst` 都引用了列表对象 `[1, 2, 3]` （如图7-1-2所示），因此，当函数内部注释（7）向该列表追加成员之后，函数外面的 `lst` 引用的列表内的成员也随之变化——内外列表本质是同一个对象。
...
调用函数时使用关键词参数，其本质与前述一样，依然是建立了“参数（变量）与对象的引用关系”。
...
分别让 `add()` 的形参 `x, y` 引用了不同类型的 Python 对象，只要它们只能能够执行 `+` 运算符，函数就能正常执行，且返回值的类型也各不相同。这就是 Python 中函数的特点（这种特征在第8章8.6节会深入探讨）：
...
>
> 从更小的范畴来说，就定义函数也要反对本本主义。自己所定义的函数，只要符合函数定义的基本格式，只要符合开发规范（不同开发团队会有各自的开发规范），只要能解决问题，就是一个“好”函数——“好”函数首先是可以用的、可以解决问题的函数，在此基础上，若能优化，再给予优化。
...
> **自学建议**
>
> 适时总结，是一种非常重要的自学方法。以本节所学习的“星号”为例，从乘法到解包操作，本书中都零零散散介绍过了。读者在学习过程中，如果觉得内容有点凌乱，很希望有人能将所有的东西总结到一张纸上的话。真正的自学者，就不要期望别人做这件事，要自己动手，才能构建起完整的、清晰的、能更新的知识结构。
...
嵌套函数，体现了函数作为对象的另一个特点：可以作为函数返回值，即 return 语句中的对象。
...
这里定义了两个函数：`out()` 和 `inner()` 。只不过这两个函数不是并列关系，而是函数 `inner()` 在函数 `out()` 里面，像这样的称为嵌套函数。
...
以7.3.2节定义的 `out()` 函数内的 `inner()` 函数为例，在 `out()` 函数所在的区域不能调用 `inner()` 函数（见7.3.2节中的报错信息），其根源即为这里介绍的作用域（scope）。每个变量或名称所引用的对象，都有各自的创建位置，也都有各自能够产生作用的区域，此区域称为作用域——在 Python 中，变量或名称的作用域，由其所在位置决定。Python 解释器会根据变量或名称定义的位置和及其在代码中的引用位置来确定作用域，以下按照搜索顺序列出各个作用域（如图7-3-2所示）：
...
在函数 `f()` 中调用 `locals()` 时，`locals()` 返回了表示函数的本地作用域的字典。注意，除了本地定义的变量 `s` 之外，本地作用域还包括函数参数 `x` 和 `y` ，它们也在 `f()` 的本地作用域内。
...
> 如何破解？
>
> 方法就是师长们谆谆教诲“要努力、要勤奋”——他们没有打诳语，倒是我们总觉得会有“成功的捷径”，甚至觉得那些都是陈词滥调，不如“名人”们热销的“我的成功可以复制”。除了天才、神童和有奇遇者之外，一般都要“闻鸡起舞奏晨曲，悬梁刺股伴灯黄”（李大钊），才能对某个方向的知识“了然于胸”，并运用自如。
...
本书不会深入介绍函数式编程（本节的命名显然有所夸大），如果对这种编程范式有兴趣，或者今后工作中会用到，建议自行阅读有关专业资料。下面只介绍与 Python 函数编程常用的几个函数，这个函数在通常的程序中也较为常见。
...
此外，参数 `func` 不必是 lambda 函数，任何函数对象均可以。
...
`filter()` 函数是 Python 的内置函数，其帮助文档显示：
